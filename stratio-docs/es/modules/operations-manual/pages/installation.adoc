...
* Bastion: requisito cgroup v2 (Kubernetes ≥ 1.35).
+
A partir de Kubernetes 1.35, la VM del _bastion_ debe tener cgroup v2 habilitado. El preflight check del kubelet (`system-validators v1.12.1`) lanza un error — no una advertencia — si detecta cgroup v1 en el host al ejecutar kubelet v1.35 o posterior. Para habilitar cgroup v2 en el _bastion_:
+
[source,bash]
----
...
----
+
Tras el reinicio, verificar con: `stat -fc %T /sys/fs/cgroup/` (resultado esperado: `cgroup2fs`).
+
