...
+
Para garantizar las funcionalidades de los _worker_ con el _control-plane_ gestionado de EKS, es necesario utilizar imágenes de sistema operativo adecuadamente configuradas. Se recomienda construir estas imágenes utilizando el https://github.com/kubernetes-sigs/image-builder[Image Builder de Kubernetes], una herramienta oficial del proyecto Kubernetes que permite crear imágenes optimizadas para diferentes proveedores cloud. Para más información sobre cómo construir imágenes personalizadas, consulte la https://image-builder.sigs.k8s.io/[documentación oficial de Image Builder].

NOTE: En su defecto, las imágenes públicas proporcionadas por AWS con el formato "amazon-eks-node-<k8s-version>-v<fecha>" son compatibles con EKS y cumplen con los requisitos necesarios para el despliegue de _Stratio KEOS_.
+
...
* Sistemas operativos certificados
+
Para garantizar las funcionalidades del _control-plane_ y los _worker_ en entornos no gestionados de Azure, es necesario utilizar imágenes de sistema operativo adecuadamente configuradas. Se recomienda construir estas imágenes utilizando el https://github.com/kubernetes-sigs/image-builder[Image Builder de Kubernetes], una herramienta oficial del proyecto Kubernetes que permite crear imágenes optimizadas para diferentes proveedores cloud. Para más información sobre cómo construir imágenes personalizadas, consulte la https://image-builder.sigs.k8s.io/[documentación oficial de Image Builder].
+
...
|<<dns, _dns_>>
|Objeto
|Configuración external DNS para _keos_.
|Ver el <<ejemplo_de_descriptor, ejemplo de descriptor>>
|Sí.
|_manage_zone_ por defecto es _true_.
...
|<<security, _security_>>
|Objeto
|Centraliza la configuración de identidades y controles de acceso para los recursos del clúster, adaptándose a los distintos proveedores cloud.
|Ver el <<ejemplo_de_descriptor, ejemplo de descriptor>>
...
En la primera ejecución, las credenciales para el aprovisionamiento en el proveedor _cloud_ se indicarán en este apartado _credentials_.
...
Las credenciales para acceder a los registros Docker y repositorios Helm se deben proporcionar en caso de que éstos requieran autenticación.
...
El bloque `security` centraliza la configuración de identidades y controles de acceso para los recursos del clúster, adaptándose a los distintos proveedores cloud. Permite definir identidades para el plano de control y los nodos, así como opciones específicas para AWS y GCP.

[cols="1,4,2,1,1"]
|===
^|Nombre ^|Descripción ^|Ejemplo ^|Opcional ^|Proveedor

|_control_plane_identity_
|Identidad (rol, cuenta de servicio, etc.) utilizada por el plano de control del clúster.
...
Cuenta de servicio predeterminada para los nodos en GKE.
|/subscriptions/6e2a38cd-../stratio-nodes / 
gke-node-sa@my-project-id.iam.gserviceaccount.com
|No / Sí
|Azure / GCP

|_aws.create_iam_
|Indica si se deben crear recursos IAM específicos para el clúster durante el aprovisionamiento.
|false
|Sí (por defecto: false)
|AWS

|_gcp.scopes_
|Lista de _scopes_ (alcances) que estarán disponibles para la cuenta de servicio de los nodos, controlando los permisos de acceso a los servicios de GCP.
a|
[source,yaml]
----
scopes:
...
Se deben indicar los repositorios Docker a los que se accederá durante la instalación. Este apartado permite configurar la URL del repositorio, su tipo y si se trata de un repositorio autenticado.
...
| _keos++_++registry_
| Indica si este repositorio es utilizado para las imágenes de Keos.
| _true_
| No. Al menos un repositorio debe ser _keos_registry_.
|===
...
- `--build-stratio-image`: La imagen de _Stratio cloud-provisioner_ no se descargará sino que se construirá y utilizará. Esta opción está destinada solo para fines de desarrollo.
- `--delete-previous`: Al establecer esta opción, el contenedor del _cluster_ local será eliminado.
- `-d, --descriptor <string>`: Permite indicar el nombre del descriptor ubicado en el directorio actual u otro directorio. Por defecto: cluster.yaml.
- `-h, --help`: Muestra la ayuda para este comando.
- `--keep-mgmt`: Mantiene la gestión del _cluster_ en kind (sólo para entornos *no productivos*).
- `--local-stratio-image-version <string>`: Permite sobrescribir la versión local de la imagen de _Stratio cloud-provisioner_ cuando se utiliza la opción use-local-stratio-image.
- `-n, --name <string>`: Nombre del _cluster_, sobrescribe KIND_CLUSTER_NAME, config (por defecto: kind).
- `--retain`: Mantiene los nodos para depuración cuando falla la creación del _cluster_.
- `--use-local-stratio-image`: La imagen de _Stratio cloud-provisioner_ no se construirá ni descargará, y se utilizará la local.
- `--validate-only`: Solo valida el descriptor y no crea el _cluster_.
