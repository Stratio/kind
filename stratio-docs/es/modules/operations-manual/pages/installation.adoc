...
Para activar el método de autorización basado en _assume role_ en AWS una vez creado el _cluster_, sigue estos pasos:

==== Prerrequisitos

Antes de empezar, asegúrate de cumplir con lo siguiente:

* Tener un rol de IAM creado con los permisos necesarios (por ejemplo: _stratio-assume-role_).
...
* Asignar al usuario de AWS una política que le permita asumir ese rol, incluyendo la acción `sts:AssumeRole` y el ARN del rol correspondiente.
+
Ejemplo de política:
+
[source,yaml]
...
==== Pasos para habilitar _assume role_
...
. Asociar el `AWSClusterRoleIdentity` al `AWSManagedControlPlane`.
+
[source,bash]
-----
...
. Configurar el `iam_role` en el _ConfigMap_ `aws-auth` con el grupo de Kubernetes correspondiente.
+
[source,bash]
-----
...
* Crear un `ClusterRoleBinding` para el grupo `capa-manager`.
+
[source,bash]
-----
kubectl apply -f - <<EOF
...
. Conceder permisos adicionales al grupo `capa-manager`:
+
[source,bash]
-----
