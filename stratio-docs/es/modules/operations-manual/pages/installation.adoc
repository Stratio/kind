...
En el caso de AWS, si deseas utilizar "assume role" como método de autorización temporal, puedes especificarlo mediante el parámetro `role_arn`. Por ejemplo:
...
NOTE: El campo `role_arn` es opcional y solo se usará dicho método en caso de definirlo en el descriptor.
