...
== Activación de ECR _pull-through cache_ en _clusters_ existentes

Si tienes un _cluster_ EKS existente y quieres activar el uso de ECR como _pull-through cache_, puedes usar el _script_ `ecr_pull_through.py`, incluido en el contenedor `cloud-provisioner-upgrade:0.17.0-0.8.X`.

El _script_ se limita a activar el _flag_ en el _cluster_:

* Actualiza _Stratio Cloud Provisioner_ al _cluster-operator_ de la versión 0.6.x.
* Activa el campo `ecr_pull_through_cache_enabled` en el `KeosCluster`.

NOTE: La migración de las referencias de imagen de los componentes del _cluster_ (CAPI, CAPA, _cert-manager_, Flux, Calico, Tigera) la realiza `upgrade-provisioner.py` durante el proceso de actualización a 0.8.X.

=== Prerrequisitos

Antes de ejecutar el _script_, verifica que la cuenta de AWS cumple los requisitos indicados en la sección xref:operations-manual:installation.adoc#_prerrequisitos_para_ecr_pull_through_cache_enabled[Prerrequisitos para `ecr_pull_through_cache_enabled`] del manual de instalación.
...
docker run --rm -it \
    ...
    cloud-provisioner-upgrade:0.17.0-0.8.X \
    python3 upgrade/resources/scripts/ecr_pull_through.py [OPTIONS]
----
