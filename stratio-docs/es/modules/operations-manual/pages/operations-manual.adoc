...
En clústeres gestionados con Cluster API Provider GCP (CAPG), el propio controlador de CAPG responsable del recurso `GCPManagedMachinePool` puede entrar en un bucle de reconciliación infinito si detecta discrepancias entre las etiquetas (`additional_labels`) definidas en la especificación del objeto (o no están especificadas) y las etiquetas realmente presentes en el `node-pool` de GKE. Este comportamiento se observa en los logs con mensajes repetidos como "Node pool config update required" y "Node pool config updating in progress". Se trata de un problema conocido en CAPG, relacionado con la reconciliación de etiquetas en los `node-pool` gestionados por el objeto `GCPManagedMachinePool`.
