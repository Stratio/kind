...
NOTE: En caso de actualización del usuario que hace el _assume role_, se deberá actualizar el secreto _capa-manager-bootstrap-credentials_ (para capa) y el secreto _keoscluster-settings_ (para el cluster-operator) con las nuevas credenciales y su _role_arn_ si fuera necesario.
