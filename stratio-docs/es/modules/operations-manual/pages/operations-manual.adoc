...
CAUTION: En algunos escenarios, los _Pod Disruption Budgets_ (PDB) pueden configurarse incorrectamente. Esto sucede cuando el PDB exige al menos una réplica disponible, pero el recurso tiene solo una réplica desplegada. Como resultado, esa única réplica no puede eliminarse, impidiendo el drenaje del nodo donde se encuentra. Este problema suele afectar a las actualizaciones, ya que durante estos procesos es común drenar nodos. Para evitarlo, asegúrate de que los despliegues tengan *más de una réplica* si el PDB requiere al menos una disponible. Antes de actualizar el _cluster_, revisa esta configuración para evitar bloqueos. Si el recurso tiene solo una réplica, una posible solución es *eliminar temporalmente el PDB* para permitir la actualización.
