...
Este comando generará el fichero con el kubeconfig donde indique la variable de entorno KUBECONFIG. Por defecto será en $HOME/.kube/config.
...
* En entornos productivos y especialmente en azure no gestionado, deberá extremarse la precaución. En particular, antes de actualizar, se recomienda hacer un _backup_ de los objetos que gestionan la infraestructura y de los servicios considerados críticos.
