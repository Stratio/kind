...
Para actualizar la versión de _Stratio Cloud Provisioner_ de 0.2 a 0.3 se debe ejecutar el _script_ _upgrade-provisioner++_++v0.3.py_.
[NOTE]
Debemos posicionarnos en el _workspace_ del _cluster_ donde se encuentran los ficheros _cluster.yaml_ y _secrets.yml_.

Podemos consultar la ayuda del _script_ con el siguiente comando:
...
** xref:attachment$upgrade-provisioner_v0.3.py[Descargar script de actualización]
