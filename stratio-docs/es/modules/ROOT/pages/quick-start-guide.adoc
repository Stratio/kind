...
==== Requisitos _workload_identity_

Para permitir que los componentes del clúster (por ejemplo, el controlador de Flux) puedan autenticarse contra servicios de Google Cloud (como Artifact Registry) tras la activación de _Workload Identity_, es necesario configurar una cuenta de servicio de GCP y vincularla con la cuenta de servicio de Kubernetes (KSA) que utiliza el componente.

A continuación se detallan los pasos necesarios para configurar una cuenta de servicio en GCP y vincularla con la cuenta de servicio de Kubernetes:

. *Crear la cuenta de servicio de GCP*
+
Si no existe, se debe crear una _Service Account_ en el proyecto de GCP que representará a la carga de trabajo.
+
[source,console]
----
...
. *Crear un rol personalizado con permisos mínimos*
+
Es una buena práctica crear un rol específico que solo contenga los permisos necesarios (en este caso, descarga de artefactos).
+
[source,console]
----
...
+
Se vincula el rol creado en el paso anterior a la cuenta de servicio de GCP.
+
[source,console]
----
...
+
Este paso autoriza a la _Service Account_ de Kubernetes (KSA) a actuar como la _Service Account_ de GCP (GSA).
+
[source,console]
----
...
+
Donde:
* `<project_id>.svc.id.goog`: Es el _pool_ de identidades de carga de trabajo del clúster.
* `<k8s_namespace>`: El _namespace_ donde reside el componente (`kube-system`).
* `<k8s_sa_name>`: El nombre de la cuenta de servicio de Kubernetes (`source-controller`).
...
* _cluster++_++network_ (opcional): Define la seguridad del cluster
** _workload_++_++identity++_++config_(obligatorio): Permite que las cargas de trabajo en tus clústeres de GKE suplanten cuentas de servicio de Identity and Access Management (IAM) para acceder a los servicios de Google Cloud.
*** _enabled_(obligatorio): Habilita la identidad de la carga de trabajo en el clúster.
*** _workload++_++pool_ (obligatorio/por defecto: `spec.credentials.gcp.project_id`.svc.id.goog): Es el grupo de trabajo para adjuntar todas las cuentas de servicio de Kubernetes a los servicios de Google Cloud.
