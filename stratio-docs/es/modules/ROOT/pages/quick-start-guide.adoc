...
NOTE: Crear un bastión para proceder a la instalación.

==== Requisitos Bastión

- Docker instalado. (27.0.3 o superior)
- Imagen local: stratio-capi-image:v1.27.0

==== _control-plane_
En cuanto al _control-plane_, en el descriptor del _cluster_ se puede indicar que se trata de un *_control-plane_ gestionado* y las siguientes especificaciones han de ser incluidas:

* _cluster_network_: Define la red del cluster.  (Obligatorio)
** _private_cluster_: Define la _spec_ del cluster privado. (Obligatorio)
*** _enable_private_endpoint_: Indica si se utiliza la dirección IP interna del _master_ como _endpoint_ del clúster.  (Obligatorio: true)(Default: true)
*** _enable_private_nodes_: Indica si los nodos del clúster tienen direcciones IP internas únicamente.  (Obligatorio: true)(Default: true)
*** _control_plane_cidr_block_ (master-ipv4-cidr): Es el rango de IP en notación CIDR que se utilizará para la red del _master_. Este rango no debe superponerse con ningún otro rango en uso dentro de la red del clúster. Se aplica cuando _enabled_private_nodes_ es true y debe ser una subred /28. (Obligatorio)
* ip_allocation_policy: Representa las opciones de configuración para la asignación de IP del clúster GKE. (Si no se especifica, se usarán los valores predeterminados de GKE).
** cluster_ipv4_cidr_block: Representa el rango de direcciones IP para las IP de los pods del clúster GKE. (Si no se especifica, se elegirá el rango con el tamaño predeterminado).
** services_ipv4_cidr_block: Representa el rango de direcciones IP para las IP de los servicios del clúster GKE. (Si no se especifica, se elegirá el rango con el tamaño predeterminado).
** cluster_secondary_range_name: Representa el nombre del rango secundario que se utilizará para el bloque CIDR del clúster GKE. El rango se utilizará para las direcciones IP de los pods y debe ser un rango secundario existente asociado con la subred del clúster.
** services_secondary_range_name: Representa el nombre del rango secundario que se utilizará para el bloque CIDR de los servicios. El rango se utilizará para las IP de los servicios y debe ser un rango secundario existente asociado con la subred del clúster.

NOTE: Si los rangos de IP ya están creados, se deben usar los nombres especificados (services_secondary_range_name y cluster_secondary_range_name). Si no existen, se debe usar la notación CIDR (services_ipv4_cidr_block y cluster_ipv4_cidr_block) para crearlos, pero no se pueden usar ambos métodos simultáneamente.

* _master_authorized_networks_config_: Representa la configuración de redes autorizadas del clúster.
...
** _cidr_blocks_: Lista de bloques CIDR que se permiten acceder al _master_. (Opcional ya que gcp_public_cidrs_access_enabled es siempre true)
*** _cidr_block_: Rango de IP en notación CIDR que se permitirá acceder al _master_. (Obligatorio en el caso de que _cidr_blocks_ esté presente)
*** _display_name_: Nombre de la red autorizada. (Opcional)
** _gcp_public_cidrs_access_enabled_: Indica si se permite el acceso a las direcciones IP públicas de Google Compute Engine. (Obligatorio: false, si _enable_private_endpoint es true)(Default: false)
* _monitoring_config_: Define la monitorización del cluster.
** _enable_managed_prometheus_: Habilita la monitorización gestionada del cluster con Prometheus. (Obligatorio)(Default: false)
* _logging_config_: Define la configuración de _logging_ del cluster.
** _system_components_: Habilita el componente _system_ de _logging_. (Obligatorio)(Default: false)
** _workloads_: Habilita el componente _workloads_ de _logging_. (Obligatorio)(Default: false)

NOTE: Cualquier modificaciónde los parámetros anteriormente mencionados no tendrá efecto, sólo se aplican en tiempo de creación del cluster. 
...
* _max++_++size_: número máximo de nodos para el autoescalado (opcional).
* _labels_: etiquetas de los nodos en Kubernetes (opcional).
* _taints_: taints de los nodos en Kubernetes (opcional).
* _root++_++volume_: particularidades del disco (opcional).
** _size_: tamaño en GB (por defecto: 30GB).
...
* En caso de utilizar una infraestructura personalizada, se deberá indicar la VPC y la subnet de la región.
[source,yaml]
----
  networks:
    vpc_id: "vpc-name"
...
* La versión de Kubernetes ha de ser (1.28) y ha de estar soportada por GKE.
* Los nombres de los grupos de _worker++_++nodes_ no pueden repetirse.
...
Esta fase (aprovisionamiento e instalación de Kubernetes) deberá ejecutarse desde la máquina bastíon.
