...
*** _enable++_++private++_++endpoint_ (obligatorio/immutable; default: true): indica si se utiliza la dirección IP interna del _master_ como _endpoint_ del _cluster_.
...
* ip++_++allocation++_++policy (opcional/immutable): representa las opciones de configuración para la asignación de IP del _cluster_ GKE (si no se especifica, se usarán los valores predeterminados de GKE).
...
** _gcp++_++public++_++cidrs++_++access++_++enabled_ (default: false), si _enable++_++private++_++endpoint es "true") (default: false): indica si se permite el acceso a las direcciones IP públicas de Google Compute Engine.
