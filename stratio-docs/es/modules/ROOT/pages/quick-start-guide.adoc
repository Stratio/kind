...
** Crear una _Managed Identity_ con los roles: Contributor, AcrPull (sobre el ACR del cluster, opcional) y Managed Identity Operator. La referencia de esta identidad (Image ID) se utilizará en el descriptor del _cluster_ con el siguiente formato: _/subscriptions/<subscription_id>/resourcegroups/<nombre_resourcegroup>/providers/Microsoft.ManagedIdentity/userAssignedIdentities/<identity_name>_.
