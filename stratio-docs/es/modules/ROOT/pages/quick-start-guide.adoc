...
==== Requisitos de _Workload Identity_

Para permitir que los componentes del _cluster_ (por ejemplo, el controlador de Flux) se autentiquen frente a servicios de Google Cloud (como Artifact Registry) tras habilitar _Workload Identity_, es necesario configurar una cuenta de servicio de GCP y vincularla con la cuenta de servicio de Kubernetes (KSA) utilizada por el componente.

A continuación se describen los pasos para crear y vincular una cuenta de servicio en GCP con una cuenta de servicio de Kubernetes:

. *Crear la cuenta de servicio de GCP*.
+
Si no existe, crea una _Service Account_ en el proyecto de GCP que represente la carga de trabajo.
+
[source,console]
----
gcloud iam service-accounts create <gcp_sa_name> \
  --project=<project_id> \
  --display-name="Flux GCP Service Account"
----

. *Crear un rol personalizado con los permisos mínimos necesarios*.
+
Crea un rol específico que incluya únicamente los permisos requeridos (por ejemplo, lectura/descarga de artefactos).
+
[source,console]
----
gcloud iam roles create <role_name> \
  --project=<project_id> \
  --title="Artifact Download Only" \
  --permissions=artifactregistry.repositories.downloadArtifacts \
  --stage=GA
----

. *Asignar el rol a la cuenta de servicio*.
+
Asocia el rol creado en el paso anterior a la cuenta de servicio de GCP.
+
[source,console]
----
...
. *Permitir la suplantación de identidad (_Workload Identity_)*.
+
Autoriza a la cuenta de servicio de Kubernetes (KSA) a actuar como la cuenta de servicio de GCP (GSA).
+
[source,console]
----
...
Donde:
+
* `<project_id>.svc.id.goog` es el _pool_ de identidades de carga de trabajo del _cluster_.
* `<k8s_namespace>` es el _namespace_ donde reside el componente (por ejemplo, `kube-system`).
* `<k8s_sa_name>` es el nombre de la cuenta de servicio de Kubernetes (por ejemplo, `source-controller`).
...
NOTE: Cualquier modificación de estos parámetros solo tiene efecto durante la creación del _cluster_. Los cambios posteriores no se aplican a los _clusters_ ya existentes.

* _cluster++_++network_ (opcional): define la configuración de seguridad del _cluster_.
** _workload_++_++identity++_++config_ (obligatorio): permite que las cargas de trabajo del _cluster_ de GKE suplanten las cuentas de servicio de _Identity and Access Management_ (IAM) para acceder a los servicios de Google Cloud.
*** _enabled_ (obligatorio): habilita _Workload Identity_ en el _cluster_.
*** _workload++_++pool_ (obligatorio, valor por defecto: `spec.credentials.gcp.project_id.svc.id.goog`): _pool_ de identidades utilizado para asociar las cuentas de servicio de Kubernetes con los servicios de Google Cloud.
*** _service++_++accounts_ (obligatorio: "flux"): lista de cuentas de servicio de Google Cloud vinculadas a las cuentas de servicio de Kubernetes.
...
Creating temporary cluster "example-gke" ...
 ...
 ✓ Installing CAPx in workload cluster 🎖️
 ✓ Configuring Flux in workload cluster 🧭
 ✓ Annotating Flux ServiceAccount for Workload Identity 🔐
 ✓ Reconciling the existing Helm charts in workload cluster 🧲
