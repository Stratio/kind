...
* 1 Google Kubernetes Engine (GKE) cluster. (VPC-native).
* 1 VPC.
* 1 subnet per region.
** Primary CIDR block for the subnet. (Nodes).
** Secondary CIDR block for the subnet. (Pods & Services).
* 1 ruta de _peering_. (VPC Network peering).
* 2 rutas, para los  CIDR secundarios de los _pods_ y _services_.
* 1 Red de VPC _peering_.
* 5 Reglas de _Firewall_ de VPC (gke-<cluster-name>-<id>-[master, vms, exkubelet, inkubelet, all]).
* Máquinas virtuales para _Workers_ (según descriptor del _cluster_ y autoescalado).
...
La red de pods en GKE se configura automáticamente con el CIDR secundario de la red de _pods_ y servicios, que se obtiene de la configuración de la red de VPC al desplegar el _cluster_.
...
En este caso, el _API Server_ se expone únicamente de forma privada, por lo que solo se podrá acceder a él desde la IP asignada al endpoint privado del _cluster_.
La IP asignada al endpoint privado pertencera al rango que se especifica en el descriptor del _cluster_.
