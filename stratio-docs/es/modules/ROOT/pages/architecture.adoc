...
Del mismo modo que el Objeto _KeosCluster_, durante la fase de instalación se creará el recurso *ClusterConfig* que permite indicar configuraciones específicas de ese clúster en particular.
...
Es posible consultar su API Reference aquí:  xref:clusterconfig-reference.adoc#ClusterConfig[]

=== Elección del Cluster Operator

Por defecto, se instalará la última versión del chart disponible en el repositorio de helm indicado en el objeto KeosCluster, aunque este comportamiento se puede sobrescribir e indicar la versión que queramos instalar, indicando el valor de xref:clusterconfig-reference.adoc#ClusterConfig[].
