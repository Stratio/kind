...
$ kubectl -n $NAMESPACE patch awsmt $MACHINE_DEPLOYMENT --type merge ;-p "{\"spec\": {\"template\": {\"spec\": {\"instanceType\": "$MACHINE_TYPE"}}}}"

$ RESOURCE_VERSION=$(kubectl -n $NAMESPACE get awsmt $MACHINE_DEPLOYMENT -o json | jq -r .metadata.resourceVersion)

$ kubectl -n $NAMESPACE patch md $MACHINE_DEPLOYMENT --type merge -p "{\"spec\": {\"template\": {\"spec\": {\"infrastructureRef\": {\"resourceVersion\": \"$RESOURCE_VERSION\"}}}}}"
----

# TODO: AGREGAR ESCALADO VERTICAL PARA GCP y AzUre
...
El upgrade del _control-plane_ en entornos productivos y especialmente en flavours no-gestionados deberá hacerse extremando todas las precauciones.
...
# TODO: mover mgmt, GNPs se crean en 0.2?, verificar etcd !!!!

===== Azure no-gestionado

La GlobalNetworkPolicy creada para el _control-plane_ se deberá actualizar con las IPs obtenidas con este comando:
