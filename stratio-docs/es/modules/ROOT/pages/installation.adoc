...
# TODO: Agregar prerreqs de Azure y AKS !!

=== Azure no-gestionado

* Permisos
+
Para los despliegues en Azure no-gestionado, se necesitarán principalmente permisos en _compute_ (instancias, discos, imágenes, _routers_, redes, etc.). Al igual que con otros proveedores soportados, el aprovisionamiento requiere una cuenta con todos los permisos solicitados.
+
xref:attachment$stratio-azure-permissions.list[Descargar permisos para Azure]
...
> donde estan las policies para Azure y AKS ? son las mismas ? No. para AKS no esta. 
