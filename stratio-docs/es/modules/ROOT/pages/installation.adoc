...
* Bastion
+
Para el despliegue de _Stratio KEOS_ en GCP gestionado, se ha de realizar a través de un _bastion_ que permita la comunicación con el _cluster_ de GKE. Para ello, se deberá crear un _bastion_ en la misma red que el _cluster_ de GKE.
...
* _nodes_identity_ (Cuenta de servicio predeterminada para los nodos). (Sólo configurables en tiempo de creación del cluster).
* _scopes_ (La lista de alcances que estarán disponibles para esta cuenta de servicio).
* Sin control de la zona DNS (habilitado por defecto).
* Definición de una _StorageClass_ por defecto. Este apartado es opcional.
* Características del _control-plane_: (Solo configurables en tiempo de creación del cluster).
** _cluster_network_
*** _private_cluster_.
**** _enable_private_endpoint_.
**** _enable_private_nodes_.
**** _control_plane_cidr_block_.
...
** _monitoring_config_
*** _enable_managed_prometheus_.
** _master_authorized_networks_config_.
*** _cidr_blocks_.
*** _gcp_public_cidrs_access_enabled_.
** _logging_config_.
*** _system_components_.
*** _workloads_.
* Grupos de nodos _worker_ con múltiples casuísticas:
...
spec:
  ...
  security:
    node_identity: "gke-node-sa@my-project-id.iam.gserviceaccount.com"
    gcp:
      scopes:
