...
NOTE: Se ha implementado la opción de establecer un min_size igual a cero, lo que permite que el autoscalado pueda incrementar o disminuir el número de nodos hasta alcanzar cero según sea necesario. Esta funcionalidad facilita un ahorro significativo de costos en comparación con versiones anteriores, ya que posibilita la definición de un grupo de workers sin instanciar ningún recurso en el _cloud provider_ que no sea necesario.
