...
El sistema operativo recomendado actualmente para este proveedor es Ubuntu 22.04.
+
Para GKE, el sistema operativo utilizado por defecto es Container-Optimized OS (COS) y no se deberá indicar ninguna imagen.
+
...
- `--use-local-stratio-image`: No se construira la imagen _stratio-capi-image_ y se usará la imagen local.
