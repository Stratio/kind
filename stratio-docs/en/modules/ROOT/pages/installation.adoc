...
+
For the EKS deployment you must create the following role manually: "AWSServiceRoleForAmazonEKS" with the associated policy "AmazonEKSServiceRolePolicy" (created by default in AWS).
...
* CloudFormation
+
In case you have not executed the CloudFormation stack or created the IAM requisites manually previously, you must add the parameter spec:security:aws:create_iam set to true (it is false by default).
