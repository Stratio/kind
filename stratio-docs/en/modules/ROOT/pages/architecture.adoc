...
NOTE: On EKS, the `keos` StorageClass uses the CSI provisioner `ebs.csi.aws.com` and sets `encrypted: true` with the AWS-managed KMS key alias `aws/ebs` by default. To use a customer-managed key, set `spec.storageclass.encryption_key` in the cluster descriptor.
