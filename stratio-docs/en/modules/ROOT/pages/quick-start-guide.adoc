...
==== _Workload Identity_ requirements

To allow cluster components (for example, the Flux controller) to authenticate against Google Cloud services (such as Artifact Registry) after enabling _Workload Identity_, you must configure a GCP service account and bind it to the Kubernetes Service Account (KSA) used by the component.

The following steps describe how to create a GCP service account and link it to a Kubernetes service account:

. * Create the GCP service account*.
+
If it does not already exist, create a service account in the GCP project that will represent the workload.
+
[source,console]
----
...
+
Create a dedicated role with only the necessary permissions (e.g., artifact read/download).
+
[source,console]
----
...
+
Bind the role created in the previous step to the GCP service account.
+
[source,console]
----
...
. *Enable identity impersonation (_Workload Identity_)*.
+
Authorize the Kubernetes Service Account (KSA) to act as the GCP Service Account (GSA).
+
[source,console]
----
...
Where:
+
* `<project_id>.svc.id.goog` is the cluster’s workload identity pool.
* `<k8s_namespace>` is the namespace where the component resides (`kube-system`).
* `<k8s_sa_name>` is the Kubernetes service account name (`source-controller`).
...
* _cluster++_++network_ (optional): defines the cluster security configuration.
** _workload_++_++identity++_++config_ (required): allows GKE cluster workloads to impersonate Identity and Access Management (IAM) service accounts to access Google Cloud services.
*** _enabled_ (required): enables _Workload Identity_ in the cluster.
*** _workload++_++pool_ (required, default: `spec.credentials.gcp.project_id.svc.id.goog`):identity pool used to associate Kubernetes service accounts with Google Cloud services.
*** _service++_++accounts_ (required: "flux"): list of Google Cloud service accounts linked to Kubernetes service accounts.
