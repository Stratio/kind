...
. Update the credentials in cluster-operator:
+
...
. Update credentials in cluster-operator:
+
* Extract the values of the cluster-operator chart:
+
[source,console]
----
helm get values -n kube-system cluster-operator -a > /tmp/cluster-operator-current-values.yml
