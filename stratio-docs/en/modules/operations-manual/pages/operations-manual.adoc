...
For the generation of custom images in AWS manages, the next xref:operations-manual:image-builder/aws-image-builder.adoc[reference documentation] has been created.
...
NOTE: Unmanaged providers will have one _MachineHealthCheck_ for the _control-plane_ and another for the _worker_ nodes, while managed ones (EKS) will only have the second one.
