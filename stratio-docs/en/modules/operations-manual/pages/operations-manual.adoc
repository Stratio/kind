...
CAUTION: In some scenarios, Pod Disruption Budgets (PDBs) may be misconfigured. This happens when the PDB requires at least one available replica, but the resource has only one deployed. As a result, that single replica cannot be removed, preventing the node it runs on from being drained. This issue mainly affects updates, as node draining is common during these processes. To avoid this, ensure that deployments have *more than one replica* if the PDB requires at least one available. Before updating the cluster, review this configuration to prevent disruptions. If the resource has only one replica, a possible solution is to *temporarily remove the PDB* to allow the update to proceed.
