...
[CAUTION]
====
Modifying the `additional_labels` field in the `worker_nodes` of a `KeosCluster` managed by CAPG will trigger the recreation of the nodes associated with the node pool. This means that the current nodes will be deleted and replaced by new ones with the updated configuration. Make sure to plan this operation to avoid potential issues.
====
