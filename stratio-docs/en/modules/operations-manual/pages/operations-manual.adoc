...
=== Reconciliation loop in GKE due to automatic labels

In clusters managed with the Cluster API Provider GCP (CAPG), the CAPG controller responsible for the `GCPManagedMachinePool` resource may enter an infinite reconciliation loop if it detects discrepancies between the labels (`additional_labels`) defined (or not defined) in the resource specification and the labels actually present in the GKE node pool. This behavior can be observed in the logs, with repeated messages such as "Node pool config update required" and "Node pool config updating in progress". This is a CAPG known issue related to label reconciliation in node pools managed by the `GCPManagedMachinePool` object.

The leading cause is that Google Cloud GKE automatically adds labels to node pools (for example, `goog-gke-node-pool-provisioning-model: on-demand`) that are not present by default in the `GCPManagedMachinePool` resource generated by Cluster API. This value is only an example: it can be any label added by Google.

To solve this issue and stop the loop, the label must be manually added to the `KeosCluster` configuration to match what the provider applies. You can follow these steps:

. Edit the `KeosCluster` resource:
+
[source,bash]
----
...
. Add the label under the _additional++_++labels_ field of the `worker_nodes` definition:
+
[source,yaml]
----
spec:
...
CAUTION: Modifying the _additional++_++labels_ field under `worker_nodes` of a `KeosCluster` managed by CAPG will cause the nodes associated with the node pool to be recreated. This means that current nodes will be removed and replaced with new ones using the updated configuration. It is recommended to plan this operation carefully to avoid downtime or service disruption.
