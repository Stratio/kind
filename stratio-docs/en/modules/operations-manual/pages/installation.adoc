...
Para habilitar el método de autorizacón _assume role_ en AWS poestriormente a la creación del _cluster_, debemos de realizar los siguientes pasos:

==== Pre-requisitos:
* IAM Role creado con permisos necesarios en AWS (e.g. `stratio-assume-role`).
. Tipo de role "AWS Account"
. Los permisos son los referenciados en los pre-requisitos de EKS.
* Configurar la _Trust Relationship_ del rol para permitir que el usuario con el que hemos desplegado el cluster pueda asumirlo (por ejemplo: _arn:aws:iam::123456789012:user/stratio-user_).
====
...
----
====
* Asociar una política al usuario de AWS que permita asumir el rol creado anteriormente. Esta política debe incluir la acción `sts:AssumeRole` y el ARN del rol.
====
Ejemplo de política:
[source,yaml]
----
...
Nota: La versión mínima de _cluster-operator_ necesaria para utilizar este método de autorización es la 0.5.2.

====
==== Pasos para habilitar el método de autorización _assume role_:

* Crear el objeto `AWSClusterRoleIdentity`.
[source,bash]
-----
...
spec:
  roleARN: arn:aws:iam::963353511234:role/capa-management-components
...
* Asociar el objeto `AWSClusterRoleIdentity` al `AWSManagedControlPlane`.
[source,bash]
-----
...
* Reiniciar el _deployment_ del _capa-controller-manager_ para que los cambios surtan efecto.
[source,bash]
-----
kubectl rollout restart deployment capa-controller-manager -n capa-system
-----

* Configurar el _iam_role_ en el _configmap_ de `aws-auth` con un grupo de Kubernetes que tenga los permisos necesarios.
[source,bash]
-----
...
data:
  mapRoles: |
    - groups:
      ...
      - system:nodes
      rolearn: arn:aws:iam::963353511234:role/nodes.cluster-api-provider-aws.sigs.k8s.io
      username: system:node:{{EC2PrivateDNSName}}
    - groups:
      - capa-manager
      rolearn: arn:aws:iam::963353511234:role/capa-management-components
      username: capa-management-components
'
-----

* Crear ClusterRoleBinding para el grupo `capa-manager`
[source,bash]
-----
kubectl apply -f - <<EOF
...
* Otorgar los siguientes permisos adicionales al grupo `capa-manager`:
[source,bash]
-----
...
kubectl auth can-i get nodes --as=capa-management-components --as-group=capa-manager
kubectl auth can-i list nodes --as=capa-management-components --as-group=capa-manager
kubectl auth can-i list pods --as=capa-management-components --as-group=capa-manager
kubectl auth can-i update daemonsets --as=capa-management-components --as-group=capa-manager
-----
