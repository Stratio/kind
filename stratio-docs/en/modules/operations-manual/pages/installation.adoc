...
====
==== Pasos para habilitar _assume role_ en el cluster de _EKS_:

NOTE: Sustituye la cuenta, el nombre del cluster `eks-cl01` y el namespace `cluster-eks-cl01` por los correspondientes a tu instalación.
...
. Actualizar la versión del _cluster-operator_ a la versión 0.5.2 o superior.
+
- Actualizar el _helmrelease_ asociado al _cluster-operator_ a la versión 0.5.2 o superior
- Actualizar el _configmap_ asociado al _cluster-operator_ a la versión 0.5.2 o superior
- Parchear el secreto  keoscluster-settings con el _role_arn_ del role que hemos creado
[source,bash]
------
...
------
. Verificar la configuración y los permisos:
+
[source,bash]
-----
