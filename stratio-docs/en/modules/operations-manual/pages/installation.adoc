...
|_additional_labels_
|Optional set of additional labels to be added to node groups (`node_pool`), in addition to the default labels. +
This field is exclusive to GKE.
a|
...
- `goog-gke-node-pool-provisioning-model`: "on-demand". The provisioning model of the node pool. 

NOTE: Depending on your case, you may need to manually add the `goog-gke-node-pool-provisioning-model` label in the descriptor if your configuration requires it at cluster deployment time to avoid reconciliation loops, or afterwards by modifying the `keoscluster` object. Nodes can be on-demand, reserved, or Spot VMs.

WARNING: These automatic labels can cause a reconciliation loop in the `cluster-api-provider-gcp` (capg) controller for node pools. See the xref:operations-manual:operations-manual.adoc#_bucle_de_reconciliación_en_gke_por_etiquetas_automáticas[Troubleshooting] section of the operations manual for more information.

You must not edit or remove these automatic labels, as GKE will automatically reconcile them by recreating the nodes associated with the corresponding node pools.

For more information about the automatic labels applied by GKE, refer to the official Google documentation: https://docs.cloud.google.com/kubernetes-engine/docs/how-to/creating-managing-labels[Automatic labels in GKE].
