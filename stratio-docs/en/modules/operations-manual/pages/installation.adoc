...
* Bastion: cgroup v2 requirement (Kubernetes ≥ 1.35).
+
From Kubernetes 1.35 onwards, the bastion VM must have cgroup v2 enabled. The kubelet preflight check (`system-validators v1.12.1`) throws an error — not a warning — if cgroup v1 is detected on the host when running kubelet v1.35 or later. To enable cgroup v2 on the bastion:
+
[source,bash]
----
...
----
+
After rebooting, verify with: `stat -fc %T /sys/fs/cgroup/` (expected output: `cgroup2fs`).
+
