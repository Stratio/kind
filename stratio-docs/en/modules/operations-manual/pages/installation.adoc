...
In the case of AWS, if you want to use "assume role" as a temporary authorization method, you can specify it using the `role_arn` parameter. For example:
...
NOTE: The `role_arn` field is optional and this method will only be used if defined in the descriptor.
