...
== Enabling ECR pull-through cache on existing clusters

If you have an existing EKS cluster and want to enable ECR as a pull-through cache, you can use the `ecr_pull_through.py` script included in the `cloud-provisioner-upgrade:0.17.0-0.8.X` container.

The script only enables the flag on the cluster:

* Upgrades _Stratio Cloud Provisioner_ to the _cluster-operator_ version 0.6.x.
* Enables the `ecr_pull_through_cache_enabled` field in the `KeosCluster`.

NOTE: The migration of the cluster components' image references (CAPI, CAPA, _cert-manager_, Flux, Calico, Tigera) is performed by `upgrade-provisioner.py` during the upgrade to 0.8.X.
